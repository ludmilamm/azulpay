...
fun UserRemoteModel.toCacheModel(token: String, refreshToken: String) = UserCacheModel(id.string, token, refreshToken, name.string, email.string, phone.string, image.string)

fun UserRemoteModel.toDomainModel() = User(id.string, name.string, email.string, phone.string, image.string)

fun TransactionResponseRemoteModel.toDomainModel(date: Long) = Transaction(BigDecimal.valueOf(value.double),
