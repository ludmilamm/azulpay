...
open class SingleEvent<out T> {
    ...
    data class Error<out T>(val error: ErrorDialogDisplayModel?) : SingleEvent<T>()
...
fun <T> LiveEvent<SingleEvent<T>>.postError(error: ErrorDialogDisplayModel) = postValue(SingleEvent.Error(error))
